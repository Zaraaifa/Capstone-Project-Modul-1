...
def calculate_data(database):
    ...
    max_pendapatan = max(data['Pendapatan'] for data in database)  # mencari max pendapatan
    min_pendapatan = min(data['Pendapatan'] for data in database)  # mencari min pendapatan#
    total_data = len(database)  # mencari total data
    jmlh_miskin = sum(1 for data in database if data['Status'] == 'Miskin')  # mencari jumlah miskin
    jmlh_tidak_miskin = total_data - jmlh_miskin  # mencari jumlah tidak miskin
    ...
    for data in database:
        ...
        jumlah_terdata[kelurahan] = jumlah_terdata.get(kelurahan, 0) + 1  # mencari kelurahan
...
def show_dashboard(database):
    ...
    """
    while True:
        hasil = calculate_data(database)  # memanggil fungsi calculate_data
        if hasil is None:  # jika hasil bernilai None, maka program akan kembali ke menu utama
        ...
        console.print(summary_table)  # menampilkan table
        ...
        if konfirmasi("Apakah ingin kembali ke menu utama? (y/n) "):
